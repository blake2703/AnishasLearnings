...
class BSTree {
    ...
    public:
        ...
        /* For all printing orders, each node should be displayed as follows:
           ...
           */
        void preOrder() const; //root, left side, right side
        void postOrder() const; //left to right starting from bottom
        void inOrder() const; //smallest to largest
        //printnode function
        void inOrder(Node *userNode) const;

        //helper functions 
        bool isEmpty();
        void setRoot(const string &userWord);
        void setNode(Node *userNode); //use when setting anything but a root 
        Node* getRoot();
        void deleteTree(Node *userNode);
        //helper function for search function --> returns node that was found
        Node* retrieveNode(const string &userString) const;
        //find height given a node 
        int findHeight(Node* keyNode) const;
        //recursive function for removal- call in remove function
        void keyRemove(Node* keyNode);
        //helper function for preOrder
        void preOrderRecursive(Node *keyNode) const;
        //helper function for postOrder
        void postOrderRecursive(Node *keyNode) const;
        //helper function for remove
        Node* getParent(Node* keyNode) const;
};
